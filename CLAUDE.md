...
- **harbour-nav widget — single source of truth (2026-05-27)**: source lives in `packages/auth/harbour-nav-vanilla.tsx`; build artifact at `apps/harbour-nav-cdn/public/harbour-nav-widget.js`; served in production by the `wv-harbour-nav-cdn` Worker via specific CF routes on `windedvertigo.com/harbour-nav-widget.js` and the `www.` variant. These routes beat wv-site's `/*` catchall (confirmed by throwaway-worker precedence test on a unique path). Workflow: edit source → `npm run rebuild-nav` from repo root → `cd apps/harbour-nav-cdn && npx wrangler deploy`. **wv-site no longer ships the bundle** — `windedvertigo/site/public/harbour-nav-widget.js` was removed in the same change. No cross-repo copy step. The earlier session's belief that CF didn't honour route precedence here was actually stale CF edge cache holding wv-site's old response; the route precedence itself works (depth-chart's specific route is the long-standing proof).
