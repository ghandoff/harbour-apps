/**
 * Inner Stripe webhook handler — extracted from
 * app/api/stripe/webhook/route.ts so it can be invoked directly from
 * the admin replay route without going through the DLQ wrapper (a
 * replayed event was already marked "seen" before its prior failure was
 * DLQ'd, so the dedup short-circuit would always trigger).
 *
 * Pure function: takes a Request, returns a Response. No middleware,
 * no DLQ, no auth — those concerns live in route.ts (DLQ wrap) and
 * the admin route (Bearer check).
 */
