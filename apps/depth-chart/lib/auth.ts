...
export const auth_config: NextAuthConfig = {
  basePath: "/harbour/depth-chart/api/auth",
  ...
  adapter: {
    async createUser(data: {
      ...
      const r = await sql.query(
        ...
        [data.email, data.name ?? null, !!data.emailVerified]
      );
      const row = r.rows[0];
      return {
        id: row.id,
    ...
    async updateUser(data: { id: string; name?: string | null }) {
      ...
      const row = r.rows[0];
      if (!row) throw new Error(`user ${data.id} not found`);
      return {
        id: row.id,
        email: row.email,
        emailVerified: row.email_verified ? new Date() : null,
...
export const { handlers, auth, signIn, signOut } = NextAuth(auth_config);
