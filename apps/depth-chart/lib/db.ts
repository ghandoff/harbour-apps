...
/**
 * Neon Postgres client for depth-chart.
 *
 ...
 *
 * Mirrors the pattern in apps/harbour/lib/db.ts and
...
function getConn(): ReturnType<typeof neon> {
  if (!_conn) {
    ...
    if (!url) throw new Error("[depth-chart-db] POSTGRES_URL or DATABASE_URL required");
    _conn = neon(url);
  }
  return _conn;
}
