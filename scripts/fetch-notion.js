...
// ============================================
async function fetchHolisticSkills() {
  const propMap = config.properties.holisticSkills;
  const required = config.required.holisticSkills;
  const dbId = config.databases.holisticSkills;

  if (!dbId || dbId === 'HOLISTIC_SKILLS_PRIMER_DB_ID') {
    throw new Error('Holistic Skills database not configured — set NOTION_HOLISTIC_SKILLS_DB_ID or update notion-config.js');
  }
  ...
  for (const page of response.results) {
    if (!validatePage(page, required, 'HolisticSkills')) {
      skipped++;
      continue;
    }
  ...
  console.log('  OK Holistic Skills: ' + skills.length + ' skills' + (skipped ? ' (' + skipped + ' skipped)' : ''));
  return skills;
}
...
async function main() {
  ...
  try {
    const [quadrants, outcomes, portfolioAssets, vaultActivities, siteContent, harbourGames, holisticSkills] = await Promise.all([
      fetchQuadrants(),
      fetchOutcomes(),
      ...
      fetchHolisticSkills().catch(err => {
        console.warn('  Warning: Holistic Skills sync skipped (' + err.message + ')');
        return [];
      }),
    ]);
    ...
    // Write Holistic Skills Primer (skip if fetch failed gracefully)
    let holisticSkillsPath = null;
    if (holisticSkills.length > 0) {
      const harbourDataDir = path.join(__dirname, '..', 'apps', 'harbour', 'data');
      if (!fs.existsSync(harbourDataDir)) {
        fs.mkdirSync(harbourDataDir, { recursive: true });
      }
      holisticSkillsPath = path.join(harbourDataDir, 'holistic-skills.json');
      fs.writeFileSync(holisticSkillsPath, JSON.stringify(holisticSkills, null, 2));
    }
    ...
    if (holisticSkillsPath) {
      console.log('  Holistic Skills: ' + holisticSkills.length + ' skills → ' + holisticSkillsPath);
    } else {
      console.log('  Holistic Skills: skipped (database not configured or not accessible)');
    }
