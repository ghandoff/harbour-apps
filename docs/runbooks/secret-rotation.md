...
- **2026-07-03 — ANTHROPIC_API_KEY (exposed in chat):** an API key was pasted into a Claude chat during a canary secret-setup step, so it was treated as compromised. Recovery followed the runbook exactly: minted a fresh key in the Anthropic console → updated the `winded.vertigo` 1Password `ANTHROPIC_API_KEY` item → ran `node scripts/rotate-secret.mjs --secret=ANTHROPIC_API_KEY` (new key onto both creaseworks workers + local mirror, re-probe verified, values stdin-only) → confirmed a fresh live AI call succeeded post-rotation → deleted the old key(s) in the console. Reinforced rule: **never paste a secret into chat.** The sanctioned path is the propagator (1Password → stdin), which never exposes the value; when a canary needs a key, `op read "op://…" | wrangler secret put …` keeps it out of chat too (and delete it from the preview worker after).
