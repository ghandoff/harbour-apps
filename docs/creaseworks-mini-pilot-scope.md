...
only one is named in the transcript: **character from a crease** (`character-from-a-crease`, confirmed ready in db) — jamie's top-rated, and the designated **fallback activity**: "whatever you collect is right."

the other four come from the top of jamie's keep/strengthen table (generated by running all playdates against his framework + a UDL exclusion checklist). **jamie's 10:03 slack post in #whirlpool is an attachment the API can't read — the four remaining names need to be confirmed from that table or the crease.works co.design miro board** (`miro.com/app/board/uXjVI4z75mc=`).

> ⚠️ **open input #1**: the 4 remaining activity slugs. garrett notes "most of those green ones are ones that I actively as a human went in and tweaked."

db candidates that match the "ages 4–6, everyday materials, ready status" profile (for cross-checking against jamie's table): `color-walk-bingo`, `material-museum`, `obstacle-course-from-couch-cushions`, `paper-plate-masks`, `story-dice-from-scraps`, `texture-rubbing-gallery`, `cup-stack-challenge`, `mud-kitchen-menu`, `shadow-tracker`, `sensory-bottle-composer`. (59 playdates total in `playdates_cache`.)
...
1. **the 4 remaining activities** — from jamie's table (slack attachment unreadable via API; check the attachment or the co.design miro).
